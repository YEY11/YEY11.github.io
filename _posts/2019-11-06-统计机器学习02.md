...
* 参数方法（“参数估计”）
  ...
  * 点估计 $\hat \theta (x1_,x_2,...,x_n)$ 是数据的一个函数（或者统计量）
* 例子
  * 抛 $n$ 次硬币，求正面朝上的概率
  * 选择一种分类器
...
* 一个设计估计量的一般原则
* 涉及优化
* $\hat\theta(x_1,...,x_n) \in \argmax\limits_{\theta\in\Theta} \prod_{i=1}^{n}p_\theta(x_i)$ （乘积是因为假设数据点之间互相独立）
* MLE 估计量是相合的（在一定条件下）
...
* 决策规则 $\delta(\bm{x}) \in A$，其中 $A$ 表示行动空间
...
### 2.2 风险与经验风险最小化（ERM）
* 在决策理论中，我们真正关心的是期望损失
* 损失 $R_\theta[\delta]=E_{\bm{X}\sim \theta}[l(\delta(\bm{X}),\theta)]$
  * 例如：真实测试误差，或者泛化误差
* 目标：选择 $\delta$ 使得 $R_\theta[\delta]$ 最小
* 无法直接完成，为什么？
* ERM：使用训练集 $\bm{X}$ 来近似估计 $p_\theta$
...
#### 例子
* 我们对 $X|\theta$ 建模为 $N(\theta,1)$，先验为 $N(0,1)$
...
* 但是，还是有一些通用的方法
  ...
  * 后验模式： $\argmax\limits_\theta P(\theta\mid X)$ (最大化后验概率 MAP)
  * 这些是贝叶斯决策理论的解释 
  <img src="https://tva1.sinaimg.cn/large/006y8mN6ly1g8cvty5gc4j30li0b4my8.jpg" width="50%">

### 3.4 贝叶斯上下文中的 MLE
* MLE 公式：寻找对数据拟合最好的参数
  $\hat\theta\in\argmax\limits_\theta P(X=x\mid \theta)$
* 在贝叶斯公式下，考虑 MAP
  $\begin{aligned}
  \hat\theta &\in \argmax\limits_\theta P(\theta\mid X=x)\\
             &= \argmax\limits_\theta \dfrac{P(X=x\mid \theta)P(\theta)}{P(X=x)} \\
             &= \argmax\limits_\theta P(X=x\mid \theta)P(\theta)
  \end{aligned}$
