...
#### 回归的概率模型
* 假设一个概率模型：$Y=\bm{X'w}+\varepsilon$
  * 这里，$\bm{X},Y$ 和 $\varepsilon$ 都是随机变量
  * 变量 $\varepsilon$ 编码了噪声
* 接下来，假设高斯噪声（独立于 $\bm{X}$）：$\varepsilon \sim N(0,\sigma^2)$
* 回忆：$N(x;\mu,\sigma^2)\equiv \dfrac{1}{\sqrt{2\pi\sigma^2}}\exp\left(-\dfrac{(x-\mu)^2}{2\sigma^2}\right)$
* 因此，
  $p_{\bm{w},\sigma^2}(y|\bm{x})=\dfrac{1}{\sqrt{2\pi\sigma^2}}\exp\left(-\dfrac{(y-\bm{x'w})^2}{2\sigma^2}\right)$

#### 参数概率模型
* 采用简化表示，判别式模型为：
  $p(y|\bm{x})=\dfrac{1}{\sqrt{2\pi\sigma^2}}\exp\left(-\dfrac{(y-\bm{x'w})^2}{2\sigma^2}\right)$
* 未知参数：$\bm{w}$
* 给定观测数据 $\{(\bm X_1,Y_1),...,(\bm X_n,Y_n)\}$，我们希望找到能够 “最好地” 解释这些数据的参数值。
* 最大似然估计：选择使得观测数据的概率最大化的参数值。

#### 最大似然估计
* 假设数据点之间互相独立，观测数据的概率为：
  $p(y_1,...,y_n|\bm x_1,...,\bm x_n)=\prod_{i=1}^{n}p(y_i|\bm x_i)$
  <br />
* 其中，$p(y_i|\bm x_i)=\dfrac{1}{\sqrt{2\pi\sigma^2}}\exp\left(-\dfrac{(y_i-\bm x_i' \bm w)^2}{2\sigma^2}\right)$
  <br />
* “Log 技巧”：相比直接求上式的最大值，我们选择求上式的对数的最大值：<br />
  $\sum_{i=1}^{n}\log p(y_i|\bm x_i)=-\dfrac{1}{2\sigma^2}\sum_{i=1}^{n}(y_i-\bm x_i' \bm w)^2+C$  <br />
  这里，$C$ 是一个常量，与 $\bm w$ 无关。
  <br />
* 在该模型下，最大化以 $\bm w$ 为变量的对数似然函数，等价于最小化平方和误差。
...
* 训练 = 拟合 = 参数估计
* 典型公式
  $\hat\bm\theta\in\argmin\limits_{\bm\theta\in\bm\Theta}L(data,\bm\theta)$
  * $\argmin$ 是因为我们希望最小化而非最小值
    * 注意：$\argmin$ 可能返回一个集合（最小化并非总是唯一的）
  * $\bm\Theta$ 表示一个模型簇（包含约束）
  * $L$ 表示某个需要优化的目标函数
    * 例如：MLE 中的（条件）似然函数
    * 例如：决策理论中的（正则化）经验风险
...
* 通过迭代求近似解
  1. 初始化：选择参数的初始值 $\bm\theta^{(1)}$，设置 $i=1$
  2. 更新：$\bm\theta^{(i+1)}\leftarrow SomeRule[\bm\theta^{(i)}]$，设置 $i\leftarrow i+1$
  3. 终止：决定是否停止
  4. 回到第 2 步
  5. 停止：返回 $\hat\bm\theta\approx\bm\theta^{(i)}$
...
* 假设 $\bm\theta=[\theta_1,...,\theta_K]'$
1. 选择 $\bm\theta^{(1)}$ 和某个 $T$
2. 对 $i$ 从 $1$ 到 $T$（也可以采用其他终止条件）：
    1. $\bm\theta^{(i+1)}\leftarrow\bm\theta^{(i)}$
    2. 对 $j$ 从 $1$ 到 $K$：
        1. 固定 $\bm\theta^{(i+1)}$ 部分，除了第 $j$ 项
        2. 找到使 $L\left(\theta_j^{(i+1)}\right)$ 最小化的 $\hat\theta_j^{(i+1)}$
        3. 更新 $\bm\theta^{(i+1)}$ 的第 $j$ 项
3. 返回 $\hat\bm\theta\approx\bm\theta^{(i)}$

### 3.5 回忆：梯度
<img src="https://tva1.sinaimg.cn/large/006y8mN6ly1g8d6yhgcz6j30aw0cugnx.jpg" width="25%" align="right">

* $\bm\theta$ 处的梯度定义为 $\left[\dfrac{\partial L}{\partial\theta_1},...,\dfrac{\partial L}{\partial\theta_p}\right]'$ 在 $\bm\theta$ 处的值。
* 梯度指向从 $\bm\theta$ 点出发，$L(\bm\theta)$ 变化最大的方向。
* 速记表示：
  * 计算在 $\bm\theta$ 点处的 $\nabla L\overset{\text{def}}{=}\left[\dfrac{\partial L}{\partial\theta_1},...,\dfrac{\partial L}{\partial\theta_p}\right]'$
  * 这里，$\nabla$ 是 “竖琴” 符号

### 3.6 梯度下降
<img src="https://tva1.sinaimg.cn/large/006y8mN6ly1g8d70xpghnj30i60j8n0s.jpg" width="40%" align="right">

* 假设 $L$ 是可微的
1. 选择 $\bm\theta^{(1)}$ 和某个 $T$
2. 对 $i$ 从 $1$ 到 $T$（也可以采用其他终止条件）：
  $\bm\theta^{(i+1)}\leftarrow\bm\theta^{(i)}-\eta\nabla L(\bm\theta^{(i)})$
3. 返回 $\hat\bm\theta\approx\bm\theta^{(i)}$
...
### 3.7 凸目标函数
<img src="https://tva1.sinaimg.cn/large/006y8mN6ly1g8d7cxg8abj30fa0gewh6.jpg" width="40%" align="right">

* “碗形” 函数
* 直观上，函数图像任意两点之间的线段位于函数图像上方。
* 定义：
$f:D\rightarrow\R$ 是凸函数，
如果 $\forall \bm{a,b}\in D,t\in [0,1]$，都有：
$f(t\bm a+(1-t)\bm b)\le tf(\bm a)+(1-t)f(\bm b)$
当不等号为严格 ( $<$ ) 时，$f$ 为严格凸函数。
此外，等效地，我们可以看一下二阶导。 对于按标量定义的 $f$，二阶导应为非负； 对于多元 $f$，Hessian 矩阵应为正半定。
* 在（严格）凸函数上使用梯度下降，可以确保找到（唯一）全局最小值。

### 3.8 $L_1$ 与 $L_2$ 范数
* 在整个课程中，我们经常会遇到范数，它们是一类具有特定形式的函数：$\R^n\rightarrow\R$
  * 直观上，范数在某种意义上衡量了向量的长度
  * 通常作为机器学习优化中的目标函数或者停止条件的一部分
* 具体来说，我们将经常使用 $L_2$ 范数（又称欧几里德距离）:
$\|\bm a\|=\|\bm a\|_2\equiv\sqrt{a_1^2+...+a_n^2}$
* 以及 $L_1$ 范数（也称为绝对范数或曼哈顿距离）:
$\|\bm a\|_1\equiv |a_1|+...+|a_n|$
* 例如，平方和误差就是一个平方范数：
$L=\sum_{i=1}^{n}\left(y_i-\sum_{j=0}^{m}X_{ij}w_j\right)^2=\|\bm y-\bm{Xw}\|^2$
...
**对于 MLE / 决策理论的推导**
### 4.1 最小二乘法
* 训练数据：$\{(\bm x_1,y_1),...,(\bm x_n,y_n)\}$ （注意：这里 $\bm x_i$ 是粗体，表示向量）
* 为了方便，行代表实例（所以，列代表属性），用一个 $n\times(m+1)$ 的矩阵 $\bm X$ 和一个 $n$ 维向量 $\bm y$ 代表训练数据
* 概率模型 / 决策规则假设：$\bm y\approx\bm{Xw}$
* 为了找到 $\bm w$，最小化平方和误差：
$L=\sum_{i=1}^{n}\left(y_i-\sum_{j=0}^{m}X_{ij}w_j\right)^2$
* 令偏导数为零，对 $\bm w$ 求解：
$\hat\bm w=(\bm{X'X})^{-1}\bm{X'y}$
  * 该方程组称为正规方程
  * 只有当矩阵的逆存在时，该方程组才是良定义的

（本教程中，粗体大写字母表示矩阵，$\bm{X'}$ 表示转置，$\bm{X}^{-1}$ 表示逆）
