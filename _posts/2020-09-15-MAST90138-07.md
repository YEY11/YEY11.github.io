...
因此，通常需要施加 $\frac{q(q-1)}{2}$ 个约束以使 $Q$ 可以被识别。有很多不同的方法可以做到这点。

一种方法是将 $Q$ 限制为 “下三角形” 矩阵，即，对于所有 $j < \ell$，都有 $q_{j\ell}=0$。因此，这里恰好有 $\frac{q(q-1)}{2}$ 个零约束。

* 通常，将进一步限制所有对角线上的元素 $q_{jj}> 0 \quad (j=1,\dots,q)$，但这并没有真正降低自由度。(整个实数轴线和一半的实数轴都是维度为 $1$ 的对象)

* 上面的约束条件是由以下版本的 **Cholesky 分解定理** 引起的：对于秩为 $q\leq p$ 的 $p\times p$ 矩阵 $A$，可以找到唯一的 $p\times q$ 矩阵 $L =(l_{j\ell})$，使得 $A = LL^{\mathrm T}$ 和 $L$ 是对角线元素大于零的下三角形矩阵，即：对于所有 $j=1,\dots,q$，都有 $l_{jj}> 0$；对于所有的 $j< \ell$，都有 $l_{j\ell} = 0$。

* (对于我们的因子模型，所讨论的 $q$ 秩矩阵 $A$ 为 $QQ^{\mathrm T}$)
...
$$\underbrace{p(p+1)/2}_{\text{对角线 } + \text{ 上三角部分约束}}$$

因子建模的目标是降维。为了避免 **“过度参数化 (over-parametrization)”**，我们通常要求

$$p(p+1)/2 \ge pq + p - q(q-1)/2$$

否则，在某些情况下，即使考虑了 $Q$ 的可识别性所需的 $q(q − 1)/2$ 个约束，也可能得到无限多的给出相同 $\Sigma$ 的 $(Q,\Psi)$ 选择。

**含义**：对于 $X$ 中给定数量 ($p$) 的观测变量，对数据建模以提供可识别模型的因子数量 ($p$) 存在上限。
...
$$l(\mathcal X; \mu,\Sigma)=-\dfrac{n}{2} \log |2\pi \Sigma| - \dfrac{1}{2}\sum_{i=1}^{n}(\overline x - \mu)^{\mathrm T}\Sigma^{-1}(\overline x - \mu)$$
...
其中，$l$ 是关于 $(Q,\Psi)$ 的函数，并且对于 $Q$ 施加了必要的约束以满足可识别性要求。
...
$$-2\log \left(\dfrac{H_0 \text{ 下的最大似然}}{\text{无模型下的最大似然}}\right)=n\log \left(\dfrac{|\hat Q \hat Q^{\mathrm T} + \hat \Psi|}{|S|}\right) \tag{4}$$

其中，$\hat Q$ 和 $\hat \Psi$ 是的 MLE 估计量 (对于这个 $q$ 而言)。

当 $H_0$ 为真时，LR 统计量具有近似卡方分布 $\chi^2_{\frac{1}{2}((p-q)^2-p-q)}$，可以用来校准模型拟合的 $p$ 值。

* 在实践中，通常将式 $(4)$ 中最前面的 $n$ 替换为 $n − 1 −(2p + 4q + 5)/ 6$ 以改良 $\chi^2$ 逼近。
* 如果必须选择其中一个，则将使用具有最大 $p$ 值的模型。
...
令 $\hat Q^* = \hat Q G$，其中 $G$ 是需要确定的正交矩阵。我们选择 $G$，使得 $\hat Q^*$ 中每一列的估计平方载荷 $$(\hat q_{j\ell}^*)$$ 的方差之和能够最大化，也就是说，我们选择 $G$ 来最大化：

$$\sum_{\ell=1}^{q}\left[\sum_{j=1}^{p}(\hat q_{j\ell}^*)^4 - \left \{ \sum_{j=1}^{p} (\hat q_{j\ell}^*)^2\right \}^2 \right]$$

* 通常，从 $X_j$ 的角度出发，得出的因子通常更容易解释。几组 $X_j$ 通常只与较少的因子相关联。
...
* 通常，未经旋转的载荷在解释时比较困难，而旋转过的载荷解释时要更容易一些。
* 第一个因子在变量 5、6、8、10、11、12 和 13 上载荷很大，并且可能代表着外向和推销员个性的人格。
* 因子 2 的权重变量 4 和 7 表示好感度。
* 因子 3 的权重变量 1、9 和 15 代表经验。
* 因子 4 和 5 各代表一个变量，分别为学习能力 3 和外表 2。
* 最后两个因子的并没有那么重要，变量 14 (加入意向) 似乎与其中几个因子相关。
...
在 PCA 中，我们的目标是显式地找到 $X$ 的线性组合。这就是我们构造 PC $Y_1,\dots,Y_p$ 的方式，并且它是模型无关的。

在因子分析中，因子无法直接计算，它们是隐变量。它们出现在对数据建模 (在协方差矩阵上) 之后。整个因子分析取决于我们假设的因子模型。如果模型是错误的，那么分析将是站不住脚的。

在因子分析中，因子并非原始变量的线性组合，它们是独立的因子，通常代表变量组可能代表的特征。

实际上，在因子分析中，我们不是将 $F$ 视为 $X$ 的函数，而是将 $X$ 表示为 $F$ 的函数。

当我们想要提取无法直接测量的特征时，这些因子的隐变量属性会特别有用。例如：焦虑因子无法被真正测量，但它可能与其他可测量的变量有关，例如出汗、考试成功、与陌生人交谈困难等等。

在 PCA 中，前几个 PC 解释了数据中的大部分变化 (这就是我们构造 PC 的方式)。

在因子分析中，$q$ 个因子通常是最容易解释的因子 (这就是我们构造因子的方式)。

有时，PCA 和因子分析会得出相似的结果，其原因是：如前所述，如果 $\Sigma$ 只有 $q < p$ 个非零特征值，那么我们可以使用前 $q$ 个 PC 来构建因子模型。
