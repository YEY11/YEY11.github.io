...
类似地，如果我们想区分正面情感和负面情感，可以使用社会心理学家（Tausczik 和Pennebaker，2010）定义的正面和负面 **情感词汇表**。
...
符号 $\text{p}(\boldsymbol x^{(i)},y^{(i)};\boldsymbol \theta)$ 表示 $\boldsymbol \theta$ 是概率函数的一个 *参数*。概率的乘积可以用对数概率之和代替，因为对数函数在正参数上单调增加，因此相同的θ将使概率及其对数最大化。 由于数值稳定性，使用对数是可取的：在大型数据集上，将许多概率相乘可能会小于零。5

通过生成模型定义概率p（x（i），y（i）；θ）-一种理想的随机过程，已生成观测数据。 6算法1描述了朴素贝叶斯分类器的生成模型，其参数为θ= {µ，φ}。
