...
>赌场内，有一名赌徒想要去摇 **老虎机（bandit）**，他面前有一排机器，每台机器都拥有一个 **臂（arm）**，而且每台机器看上去都一样。每次投一枚硬币就能获得一次 **摇臂（play）** 的机会，而且每个臂摇下都有可能获得 **奖励（rewards）** —— 但是，每台老虎机吐出奖励的概率分布是未知的。作为赌徒，自然希望自己的 **累积收益的期望（expected cumulative reward）最大化**（假如一共有 $n$ 次摇臂的机会），那么，请问这名赌徒应该采取怎样的行动？<br>  
>
...
### 1.3 随机 MAB 设定
* 可能采取的行动 $$\{1,...,k\}$$ 称为 “<span style="color:red">臂（arms）</span>”
...
* 在第 $t$ 轮：
  * 将每个臂（arm）$i$ 观测到的平均奖励（average reward）作为 <span style="color:red">估计值</span>  
    ...
    其中，在某个臂（arm）$i$ 被第一次摇到之前，其估计值都设为初始常数 $Q_0(i)=Q_0$  
...
* **<span style="color:steelblue">定理：霍夫丁不等式（Hoeffding's inequality）</span>**
  * 令 $X_1,...,X_n$ 为独立同分布的随机变量，并且 $X_i \in [0,1]$，均值为 $\mu$，它们的样本均值则由 $\bar {X_n}$
