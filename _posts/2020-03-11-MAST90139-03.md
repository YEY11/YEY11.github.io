...
我们可以将 $H$ 对角线上的第 $i$ 个元素 $h_{ii}$ 简记为 $h_i$，它反映了第 $i$ 和观测值 $y_i$ 作用在第 $i$ 个拟合值 $\hat y_i$ 上的杠杆，这也是我们最感兴趣的影响。由于 $\mathrm{tr}(H)=\sum_{i=1}^{n}h_{ii}=p$ 并且 $0\le h_{ii}\le 1$，因此，$H$ 的对角线上所有元素的平均大小为 $\dfrac{p}{n}$。
