...
<img src="http://andy-blog.oss-cn-beijing.aliyuncs.com/blog/2020-03-19-WX20200319-201812%402x.png">

**<center><span style="font-size:10pt">图 3：R 中绘制的模型诊断图</span></center>**

可以看到，`plot()` 命令生成了四张诊断图。第一张是拟合值与残差的关系图，图中的所有点基本都落在一条水平带中间，说明模型是比较满意的。第二张是正态 QQ 图，检查的是残差的正态性。第三张是拟合值与标准化残差绝对值的平方根的关系图，它比第一张简单的残差图要好一些，我们可以看到它的模式。最后一张图是每个数据点的杠杆与标准化残差的关系图，可以看到大部分的标准化残差集中在 $0$ 附近，但是也存在少数比较大的标准化残差，以及少数的高杠杆点，为了综合考虑大的标准化残差和高杠杆的影响，我们引入了库克距离。
